...
namespace DontMove
{
    public sealed class GameHUD : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            string status = game.State == GameState.MissionComplete ? "MISSION COMPLETE" : game.State == GameState.Detected ? "DETECTED" : game.DiamondBannerVisible ? "DIAMOND ACQUIRED" : game.State == GameState.Calibrating ? (game.ShowingReady ? "READY" : "HOLD COMFORTABLY") : "";
            GUI.Label(new Rect(0, h * .14f, w, 60), status, title);
            if (game.State == GameState.Calibrating && !game.ShowingReady)
            {
                center.normal.textColor=Color.white;
                string message=game.sensor.CalibrationFailed ? "Hold still, then retry calibration" : "Hold steady  " + game.sensor.CalibrationStableTime.ToString("F1") + " / " + game.sensor.stableHoldDuration.ToString("F1") + "s";
                if(!Application.isEditor && !game.sensor.HasAttitude) message="Waiting for device attitude sensor";
                GUI.Label(new Rect(0,h*.22f,w,38),message,center);
                if(game.sensor.CalibrationFailed && GUI.Button(new Rect(w*.2f,h*.29f,w*.6f,45),"RETRY CALIBRATION"))game.BeginCalibration();
            }
