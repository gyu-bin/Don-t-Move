...
namespace DontMove
{
    ...
    public sealed class PlayerController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (game.State == GameState.Freeze)
            {
                // Freeze reference is independent of guard visibility. Device motion drives intent;
                // controller deceleration is never used as evidence that the user's hand moved.
                input = game.SensorMovementScore > game.detection.movementDeadZone ? sensor.FreezeMovementVector : Vector2.zero;
            }
        ...
        public void TickMovement(Vector2 input, float dt, bool canMove = true)
        {
            ...
            MovementState = Speed <= stopThreshold ? MovementState.Idle : Speed <= minSpeed ? MovementState.Sneak :
