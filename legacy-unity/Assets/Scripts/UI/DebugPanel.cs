...
namespace DontMove
{
    public sealed class DebugPanel : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            GUILayout.Label("Smoothed input: " + sensor.SmoothedInput.ToString("F3"));
            ...
            GUILayout.Label("Dead zone output: " + sensor.DeadZoneOutput.ToString("F3"));
            ...
            GUILayout.Label("Device score: " + sensor.EvaluateDeviceMotion(detection.motionThreshold).ToString("F2"));
            ...
            detection.motionThreshold = Slider("Motion threshold (deg/s)", detection.motionThreshold, .5f, 30f);
            ...
            detection.safeDecayDelay = Slider("Safe decay delay", detection.safeDecayDelay, 0f, 5f);
            detection.deviceMotionWeight = Slider("Device motion weight", detection.deviceMotionWeight, 0f, 1f);
            ...
            if (GUILayout.Button("RECENTER NOW")) game.Recenter();
        ...
        void ApplyPreset(int preset)
        {
            ...
            sensor.deadZoneDegrees = 3f; sensor.maxTiltDegrees = 25f;
            sensor.smoothing = preset == 0 ? .08f : preset == 2 ? .04f : .06f;
            player.maxSpeed = 5.2f; player.acceleration = preset == 0 ? 12f : preset == 2 ? 22f : 15f;
            player.deceleration = preset == 0 ? 36f : preset == 2 ? 36f : 32f;
